...
async function main() {
  try {
    ...
    // 2. Find all unique questionIds in responses
    const uniqueFormIds = await responses.distinct('questionId');
    console.log('Unique Form IDs (questionId) in responses:', uniqueFormIds);

    for (const formId of uniqueFormIds) {
      const count = await responses.countDocuments({ questionId: formId });
      console.log(`Form ID (questionId): ${formId} - Responses: ${count}`);
      ...
      if (count === 24) {
        ...
        if (!existing) {
          ...
          // If it's the other form (2 responses or 0), we don't restore it to this tenant
          console.log(`NOT RESTORING Form: ${formId} (responses: ${count})`);
      }
    }
